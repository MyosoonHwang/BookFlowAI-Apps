"""mock auth middleware. Bearer mock-token-{role} -> (user_id, role) injected into request.

Phase 4: swap with real Entra OIDC JWT verification (azure-entra-mock returns JWS today).
"""
...
class AuthContext:
    ...
    def __init__(self, user_id: str, role: str, scope_wh_id: int | None, scope_store_id: int | None):
...
def require_auth(authorization: str | None = Header(default=None)) -> AuthContext:
    ...
    if not token.startswith("mock-token-"):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="non-mock token (real JWT verification not yet wired)")
